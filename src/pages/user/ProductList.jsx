    ...
    <section className="user__section">
      ...
      <div className="user__header">
        <div className="user__header-content">
          <h2 className="user__title">Our Products</h2>
          <p className="user__subtitle">Discover our curated selection of premium products</p>
        </div>
        <div className="user__search">
          <FontAwesomeIcon icon={faSearch} className="user__search-icon" />
          <input
            type="text"
      ...
      <div className="user__content">
        <aside className="user__filters">
          <div className="user__filters-header">
            <h3>
              <FontAwesomeIcon icon={faFilter} /> Filters
            </h3>
            {hasActiveFilters() && (
              <button className="user__clear-filters" onClick={handleClearFilters}>
                <FontAwesomeIcon icon={faTimes} /> Clear All
              </button>
            )}
          </div>

          <div className="user__filter-section">
            <h3 className="user__filter-title">
              <FontAwesomeIcon icon={faTags} /> Categories
            </h3>
            <div className="user__filter-options">
              {categories.map((category) => (
                <label key={category} className="user__filter-option">
                  <input
                    type="checkbox"
                  ...
                  {filters.categories.includes(category) && (
                    <FontAwesomeIcon icon={faCheck} className="user__filter-check" />
                  )}
                </label>
              ))}
            </div>
          ...
          <div className="user__filter-section">
            ...
            <div className="user__price-range">
              <div className="user__price-slider">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    // Ensure max doesn't drop below min
                    if (value >= min) {
              ...
              <div className="user__price-inputs">
          ...
          <div className="user__filter-section">
            ...
            <div className="user__rating-filter">
              {[5, 4, 3, 2, 1].map((rating) => (
                <label key={rating} className="user__filter-option">
                  <input
                    type="radio"
                    name="rating"
                  ...
                  {filters.rating === rating && <FontAwesomeIcon icon={faCheck} className="user__filter-check" />}
                </label>
              ))}
            </div>
          </div>
        ...
        <div className="user__products-container">
          <div className="user__products-header">
            <p className="user__products-count">
              Showing {startIndex + 1}-{Math.min(endIndex, totalItems)} of {totalItems} products
            </p>
            <div className="user__products-sort">
              <label htmlFor="sort-select">Sort by:</label>
              <select id="sort-select" className="user__sort-select" onChange={handleSortChange} value={sortOption}>
          ...
          <div className="user__product-grid">
            {loading
              ? renderSkeleton()
              : currentProducts.map((product) => (
                  <div key={product.id} className="user__product-card" onClick={() => handleProductClick(product.id)}>
                    <div className="user__product-card-inner">
                      {product.discount && <div className="user__product-badge">-{product.discount}% OFF</div>}

                      <button
                        className={`user__product-favorite ${favorites.has(product.id) ? 'active' : ''}`}
                        onClick={(e) => {
                          e.stopPropagation();
                      ...
                      <div className="user__product-image-container">
                        ...
                        />
                        {product.isOrganic && (
                          <div className="user__product-organic">
                      ...
                      <div className="user__product-info">
                        <div className="user__product-category">{product.category}</div>
                        <h3 className="user__product-name">{product.name}</h3>
                        ...
                        <div className="user__product-description">
                        ...
                        <div className="user__product-footer">
                          {product.inStock ? (
                            ...
                            <div className="user__product-stock out">
          ...
          {!loading && currentProducts.length === 0 && (
            <div className="user__no-products">
